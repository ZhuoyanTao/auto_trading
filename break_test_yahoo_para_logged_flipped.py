...
def backtest_single_combination(args):
    ...
    for ticker in adj_close.columns:
        ...
        for i in range(neighborhood_size, len(prices)):
            ...
            rise_from_max = (prices.iloc[i] - local_max) / local_max if local_max > 0 else 0
            drop_from_min = (local_min - prices.iloc[i]) / local_min if local_min > 0 else 0

            # Check for entry points
            if position is None:
                if rise_from_max > threshold:
                    position = 'long'
                    entry_price = prices.iloc[i]
                    peak_price = prices.iloc[i]
                    trade_count += 1
                    print(f"LONG ENTRY: {ticker} at {prices.index[i]}, Price={entry_price:.2f}")
                elif drop_from_min > threshold:
                    position = 'short'
                    entry_price = prices.iloc[i]
                    peak_price = prices.iloc[i]
                    trade_count += 1
...
if __name__ == '__main__':
    ...
    neighborhood_sizes = np.arange(1, 61, 1)  # Neighborhood sizes: 10 to 30 intervals
